...
# manual start, or a prior instance that a systemd restart did not fully
# reap) before starting a new one -- two live instances fight over the same
# GPIO lines/serial port, and the losing instance hardware init hangs
# forever. The leading "-" ignores the pkill exit code (1 = nothing
# matched), which is not a failure here.
